...
def call_nvidia(grid_path, source_duration, insights, style_prompt):
  ...
  if not api_key:
    return {"status": "failed", "error": "NVIDIA API key missing"}
...
def call_openrouter(grid_path, source_duration, insights, style_prompt):
  ...
  if not api_key:
    return {"status": "failed", "error": "OpenRouter API key missing"}
...
Return ONLY valid JSON format, no markdown wrapping."""
  ...
  payload = {
      # OpenRouter ka dynamic free router jo automatic active free vision model chun lega
...
def run_fallback_pipeline():
  ...
  # 2. Agar NVIDIA fail hua, toh OpenRouter (openrouter/free) par switch ho jao
