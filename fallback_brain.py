...
def clean_and_parse_json(raw_str):
    """Sanitizes raw response to fix leading zeros in integer numbers and extract valid JSON."""
    ...
    # Fix leading zeros (e.g., ": 015" -> ": 15", ": 00" -> ": 0")
...
def call_nvidia(grid_path, source_duration, insights, style_prompt, model_name, api_key):
    ...
    if valid_model.startswith("nvidia/"):
        valid_model = valid_model
    elif valid_model.startswith("vidia/"):
...
Return ONLY standard raw JSON format."""
    ...
    for attempt in range(1, 6):
        try:
            print(f"🔑 Trying Proxy (Attempt #{attempt}) with model {valid_model}...", file=sys.stderr)
            ...
            if response.status_code == 200:
                ...
                last_error = "Proxy 200 OK but missing 'choices'."
            elif response.status_code in [429, 503, 504]:
                wait_time = attempt * 2
                last_error = f"Proxy Error {response.status_code}: {response.text[:100]}"
            ...
            last_error = f"Proxy Exception: {str(e)}"
            time.sleep(2)
...
def run_pipeline():
    ...
    # Phase 2: NVIDIA Fallback via Worker Proxy
    if (not result or result.get("status") != "success") and nvidia_api_key:
        print("🔄 Falling back to NVIDIA API via Worker Proxy...", file=sys.stderr)
