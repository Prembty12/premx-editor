...
Return ONLY valid JSON format, no markdown wrapping."""
    ...
    # Model name tabhi payload me bhejenge agar define kiya gaya ho
    if model_name:
    ...
    for idx, api_key in enumerate(api_keys, 1):
        ...
        try:
            print(
                f"🔑 Trying API Key #{idx} with model {model_name or 'auto-default'} (Max {timeout_sec}s)...",
                file=sys.stderr,
            )
    ...
    return {
        "status": "failed",
        "error": f"Failed for model {model_name or 'auto-default'}. Last error: {last_error}",
    }
...
def run_pipeline():
    ...
    # DYNAMIC MODEL CHECK (HARDCODED NEMOTRON REMOVED)
    env_model = os.environ.get("OPENROUTER_MODEL", "").strip()
    primary_model = env_model if env_model else None
    ...
    # STEP 1: Dynamic Primary Attempt (1 Try | 10s Limit)
    print(
        f"🔄 Primary Model Check: {primary_model or 'ENV NOT SET (Skipping to Fallback standard)'} (1 Try | 10s Limit)...",
        file=sys.stderr,
    )
    if primary_model:
