    ...
    for f in all_files:
        try:
            ...
            if not content:
                ...
                shutil.move(f, archive_path)  # FIXED: Replaced os.rename with shutil.move to prevent cross-device errors
            else:
...
{{"chosen_game": "{chosen_game}", "chosen_style": "{chosen_style}", "reasoning": "Approved for high engagement"}}"""
    ...
    # Save history FIRST so screenshot paths are updated inside game_stats before saving memory
    updated_game_stats = save_professional_history(
    ...
    memory["game_stats"] = updated_game_stats

    # FIXED: Memory now saves AFTER history and screenshot paths are fully processed
    try:
    ...
    saved_screenshot_logged = updated_game_stats.get(chosen_game, {}).get("last_screenshot", "N/A")
