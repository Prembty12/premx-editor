...
# 3 Attempts (1 + 2 Retries) + Har REJECT pe shift
...
# ═════════════════════════════════════════════════════════
# 🔄 RETRY LOOP — 3 Attempts (1 original + 2 retries)
# ═════════════════════════════════════════════════════════
MAX_ATTEMPTS=3
ATTEMPT_NUM=0
FINAL_STATUS=""
...
EOF
    ...
    # 🚫 REJECT from Gemini → try Fallback
    if [ "$PARSED_STATUS" == "reject" ]; then
        echo "   🚫 GEMINI REJECTED: $PARSED_REASON"
        echo "   🔄 Trying Fallback AI (Pollinations)..."
        ...
        if [ -f "fallback.sh" ]; then
            ...
            if [ "$FB_STATUS" == "success" ]; then
                echo "   ✅ FALLBACK APPROVED"
                PARSED_JSON_DATA="$FB_RESULT"
                FINAL_STATUS="approved"
                break
            elif [ "$FB_STATUS" == "reject" ]; then
                echo "   🚫 FALLBACK ALSO REJECTED: $FB_REASON"
                PARSED_STATUS="reject"
                PARSED_REASON="$FB_REASON (fallback)"
            else
                echo "   ⚠️  Fallback failed too"
            fi
        else
            echo "   ⚠️  fallback.sh not found — skipping"
        fi
    fi

    # ❌ Gemini FAILED (parse error) — try fallback
    if [ "$PARSED_STATUS" == "failed" ]; then
        echo "   ⚠️  Gemini failed. Trying Fallback AI..."
        export GRID_PATH SOURCE_DURATION STYLE_PROMPT SELECTED_URL
        ...
        if [ -f "fallback.sh" ]; then
            ...
            if [ "$FB_STATUS" == "success" ]; then
                ...
                echo "   🚫 FALLBACK REJECTED: $FB_REASON"
                PARSED_STATUS="reject"
                PARSED_REASON="$FB_REASON (fallback)"
            else
                echo "   ❌ Both Gemini and Fallback FAILED"
                PARSED_STATUS="failed"
                PARSED_REASON="Both AIs failed"
            fi
        else
            echo "   ⚠️  fallback.sh not found — skipping"
        fi
    ...
    if [ "$PARSED_STATUS" == "reject" ]; then
        if [ $ATTEMPT_NUM -lt $MAX_ATTEMPTS ]; then
            ...
            echo ""
            echo "   🚫 3 baar REJECT ho gaya — SHIFT folder mein move kar raha hoon"
            FINAL_STATUS="shift"
            break
        fi
    fi
...
# 🚫 SHIFT case — 3 baar REJECT
if [ "$FINAL_STATUS" == "shift" ]; then
    echo ""
    echo "════════════════════════════════════════════════════"
    echo "🚫 3 BAAR REJECT — LINK KO SHIFT FOLDER MEIN MOVE"
    echo "════════════════════════════════════════════════════"
    echo "   💬 Last Reason : $PARSED_REASON"
    echo "   🔗 Link        : $SELECTED_URL"
    echo "   🎮 Game        : $SELECTED_GAME_NAME"
    echo "   🕐 Time (IST)  : $(date '+%Y-%m-%d %H:%M:%S IST')"
    echo ""
    
    echo "$SELECTED_LINE | REJECT (3x): $PARSED_REASON" >> "$GAME_SHIFT_LOG"
